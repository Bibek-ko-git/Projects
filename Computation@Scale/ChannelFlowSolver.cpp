  ...
  : NavierStokesSolver<dim>(mpi_comm)
{
  ...
  this->rho = 1.0;
  this->Re = 30.0;
  this->dt = 0.0001;       // Start with smaller time step
  this->dt_min = 0.00001;
  this->dt_max = 0.0005;
  this->T = 0.0005;
}
...
void ChannelFlowSolver<dim>::make_grid()
{
  ...
  // Partition the mesh for parallel computation (no need)
  //GridTools::partition_triangulation(this->n_mpi_processes, this->triangulation);
...
void ChannelFlowSolver<dim>::load_checkpoint()
{
  ...
  // Broadcast time and timestep
  Utilities::MPI::broadcast(this->mpi_communicator, this->timestep, 0);
  Utilities::MPI::broadcast(this->mpi_communicator, this->time, 0);
  ...
  // Broadcast local_offset (this is redundant, all processes should know their own offset)
  ...
  // Create non-ghosted vector for writing
  ...
  // Set values in the non-ghosted vector
  solution_temp.set(n_local,
